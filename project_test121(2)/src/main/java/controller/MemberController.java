...
//		session.invalidate();
	...
	public String LogoutPage(@RequestParam(value = "code", required = false, defaultValue = "") String code, HttpServletRequest request) {
		ms.logOut();
		HttpSession session = request.getSession();
		session.removeAttribute("sessionId");
		session.removeAttribute("session_age_range");
		session.removeAttribute("session_gender");
		session.removeAttribute("session_image");
		if(request.getHeader("Referer") != null) {
